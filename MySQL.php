...
class MySQL
{
     ...
     */
    public function __construct($dbConnectionData = [], $charset = null)
    {
        if (!empty($dbConnectionData)) {
            $dbserver = (!empty($dbConnectionData['dbserver'])) ? $dbConnectionData['dbserver'] : '';
            $username = (!empty($dbConnectionData['username'])) ? $dbConnectionData['username'] : '';
            $password = (!empty($dbConnectionData['password'])) ? $dbConnectionData['password'] : '';
            $database = (!empty($dbConnectionData['database'])) ? $dbConnectionData['database'] : '';
        }

        $this->db_host    = (!empty($dbserver)) ? $dbConnectionData['dbserver'] : getenv("DBSERVER");
        $this->db_user    = (!empty($username)) ? $dbConnectionData['username'] : getenv("DBUSER");
        $this->db_pass    = (!empty($password)) ? $dbConnectionData['password'] : getenv("DBPASSWORD");
        $this->db_dbname  = (!empty($database)) ? $dbConnectionData['database'] : getenv("DBNAME");
        $this->db_charset = (!empty($charset))  ? $charset : 'utf8';
    ...
    {
        $date = date('Y', strtotime((string) $value));
        if ($date == "1969" || $date == '') {
            return false;
        } else {
            return true;
     ...
     * @return boolean Returns TRUE on success or FALSE on error
     */
    public function Open($database = null, $server = null, $username = null, $password = null, $charset = "utf8", $pcon = false)
    {
        $this->ResetError();
        if (is_bool($pcon)) {
            $this->db_pcon = $pcon;
